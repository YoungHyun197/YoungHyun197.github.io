...
1) sort input int a canonical order 
   > 고차원 공간에서 point permutation에 stable한 order가 존재하지 않으므로 불가능
...
3) Use a simple symmetric function to aggregate the information from each point
   > mlp와 max pooling을 사용시, general function을 symmetric function으로 근사할 수 있고 <br/>
...
1. 3D Object Classification 
    > 12,311개의 CAD models을 사용하였으며, 대략 3/4, 1/4를 train, test set으로 분리하였음 <br>
    > input point cloud는 mesh faces에서 1024개의 point들을 sampling후 unit sphere로 normalize <br>
    > data augmentation 으로 up-axis 축을 따라 랜덤하게 rotate 혹은 값에 gaussian noise를 추가 <br>

2. 3D Object Part Segmentation
   > part segmentation은 per-point classification problem으로 간주 <br>
   > metric으로 shape의 mIoU를 사용. 겹치는 point는 IoU에 포함시켜 측정

3. 3D Part Segmentation
   > 각 point는 (X, Y, Z) 좌표와 (R, G, B) + normalized location 정보를 사용 <br>
   > point 하나당 9-dim vector  <br>
